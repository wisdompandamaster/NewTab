...
function ContactUs(){
  ...
  .then((data)=>{message.success('感谢您宝贵的意见，稍后给您回邮件')}
...
function About(){
   return (
      ...
      <div className='about-member'> 
        <div>唐  琦</div><div>时昕昱</div><div>曹  晨</div><div>李世珍</div><div>胡义越</div><div>史子奇</div>
      </div>
       <div className='codeAd'><a target={'_blank'} rel={'noreferrer'} href='https://github.com/wisdompandamaster/NewTab'><GithubOutlined /></a><div>持续更新中 ~ </div></div> 
  ...
  return (
    <div className='setting'>
      ...
      <Drawer 
        ...
        maskStyle={{backgroundColor:'rgb(0,0,0,.1)'}} title={<div style={{fontSize:'20px',    letterSpacing:'3px',fontWeight:'600',marginLeft:'10px'}}>设置</div>} placement="right" 
        onClose={onClose} 
        visible={visible}>
        <Collapse  className='setting-collapse' bordered={false} defaultActiveKey={['1']}  expandIconPosition={'right'}>
          ...
          <Panel header={<div className='panel-title'>关于我们</div>} key="2" className='setting-panel'>
          <About></About>
          </Panel>
