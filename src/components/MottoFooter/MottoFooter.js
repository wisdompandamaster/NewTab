...
const YearToday = () => {
  ...
  //那年今日 api
  let url =
    "https://cdn.jsdelivr.net/gh/Zfour/Butterfly-card-history@latest/baiduhistory/json/" +
    month +
    ".json";
...
const MottoFooter = memo(() => {
  ...
  let kinds = footerkinds.reduce((pre, cur, i) => {
    //还没加到localstorage
    return pre + "c=" + cur + "&";
  }, "");
  ...
  async function getMotto() {
    fetch(url)
      ...
      .then(data => {
        localStorage.setItem("motto", JSON.stringify(data));
        setMotto(data);
      })
  ...
  const clipMotto = () => {
    navigator.clipboard.writeText(motto.hitokoto).then(() => {
  ...
  return (
    <div
      style={{ visibility: footerexist ? "visible" : "hidden" }}
      className='footer'
    >
      <span style={{ position: "absolute", right: "3%", color: "aqua" }}>
        <Popover
          ...
          trigger='hover'
          color='#fff5'
          overlayInnerStyle={{ borderRadius: "10px", padding: "0" }}
        >
          <UnorderedListOutlined />
        </Popover>
      </span>
      <div
        style={{ display: footerType ? "none" : "inline-block" }}
        onClick={clipMotto}
        onWheelCapture={handleWheelCapture}
      >
        <span style={{ color: "#fffa", fontStyle: "italic", fontWeight: 500 }}>
          --{motto.from}--
        </span>
        <span style={{ color: "#fffa", fontStyle: "italic", fontWeight: 500 }}>
          {motto.from_who}
...
const SetFooter = memo(() => {
  const footer_api_list = [
    { value: "诗词", id: "i", disabled: true },
    { value: "动画", id: "a" },
    { value: "漫画", id: "b" },
    { value: "游戏", id: "c" },
    { value: "文学", id: "d" },
